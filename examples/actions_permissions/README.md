...
**Default Workflow Permissions**: The GitHub Terraform provider does not currently support configuring `default_workflow_permissions` (read vs write GITHUB\_TOKEN scopes) at the repository level. This setting controls whether workflows get read-only or read-write tokens by default. You must configure this through:
